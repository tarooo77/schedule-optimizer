...
def run_ultimate_optimization(num_students, max_attempts=50000):
    """希望外ゼロを目指した究極の最適化を実行"""
    print(f"\n=== 希望外ゼロを目指した究極の最適化 ===")
    ...
    print(f"希望外がゼロになるまで究極の最適化を実行します...")
    print(f"各曜日の組み合わせに対して最大{max_attempts}回の試行を行います")
    print(f"希望外ゼロが見つかるまで諦めません！")
    ...
    # 結果ファイルのパス
    results_file = os.path.join(results_dir, "ultimate_optimization_results.txt")
    
    # 結果をファイルに書き込む
    with open(results_file, 'w', encoding='utf-8') as f:
        f.write("=================================================\n")
        f.write("希望外ゼロを目指した究極の最適化の結果\n")
        f.write("=================================================\n\n")
    ...
    print(f"\n究極の最適化の詳細結果を {results_file} に保存しました。")
    
    return best_result

def main():
    """メイン処理"""
    print("=== 希望外ゼロを目指した究極の最適化 ===")
    ...
    # 徹底的な最適化の結果を表示
    print("\n【徹底的な最適化（500回×4組み合わせ）での結果】")
    print("これは既に実行済みの結果です。希望外: 3名 (14.3%)")
    
    # 究極の最適化を実行
    results = run_ultimate_optimization(21, max_attempts=5000)
