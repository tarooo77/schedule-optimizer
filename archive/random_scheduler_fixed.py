...
from utils import TIMES, get_all_slots, validate_preferences
...
def create_fully_random_data(num_students):
    ...
    ただし、有効なスロットのみを使用
    """
    # 通常のget_all_slots関数を使用して有効なスロットを取得
...
def main():
    ...
    # 結果ファイルのパス
    results_file = os.path.join(results_dir, "random_schedule_results_fixed.txt")
    
    # 結果をファイルに書き込む
    with open(results_file, 'w', encoding='utf-8') as f:
        f.write("=================================================\n")
        f.write("完全ランダムなスケジュール最適化の詳細プロセスと結果（修正版）\n")
        f.write("=================================================\n\n")
        ...
        f.write("このスケジュール最適化では、21名の生徒を火曜日から木曜日の時間帯に\n")
        ...
        f.write("現在の実装では、生徒数に基づいて利用可能なスロットが決定されます。\n")
        f.write("21名の生徒の場合、火曜日から木曜日までの3日間（各日7スロット）が使用されます。\n")
        f.write("金曜日のスロットは使用されません。\n\n")
        
        f.write("改善案:\n")
        f.write("1. 全ての曜日（火曜日から金曜日）を使用できるように修正する\n")
        f.write("2. 各曜日のスロット数を柔軟に設定できるようにする\n")
        f.write("3. 生徒数に関わらず、全てのスロットを使用できるようにする\n\n")
        
        f.write("これらの改善により、より柔軟なスケジュール最適化が可能になります。\n")
    
    print(f"\n完全ランダムな詳細結果（修正版）を {results_file} に保存しました。")
